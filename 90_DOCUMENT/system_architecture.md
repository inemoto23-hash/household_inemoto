...
BE の業務ロジックの純粋部分は `src/domain/` に隔離し、Vitest（`npm test`）で単体テストする
（[処理フロー](system_flow.md#デプロイ)）。
...
台帳ではないものが2つ、**日付にぶら下がる**形で並んでいる。`schedules`（予定と通知）と
`shopping_items`（買い物メモ）で、どちらも `entries` と接続せず集計に影響しない。
**テーブルは分けてある。** 予定は時刻・通知の宛先・`schedule_reminders` を持ち、
買い物メモは品名とチェックだけを持つ。日付にぶら下がる点が同じなだけで性質が違う
（[処理フロー](system_flow.md#買い物メモ)）。
...
**カレンダーの選択日から下は折り畳めるブロックを積む**（`components/Collapsible.tsx`）。
現在は 明細記録・予定・買い物メモ の3つで、今後さらに増える。素直に積むと下へ延々と伸び、
カレンダーを触るのにスクロールが要るようになる。開閉は端末に覚え、
**閉じているブロックは中身を描かない**（問い合わせも走らない）。
代わりに見出しへ件数を出すので、閉じていても中身の有無は分かる。

**月の切り替えはヘッダーに1つだけ置く**（`components/MonthNav.tsx`）。
カレンダー・予算・分析がそれぞれ持っていたが、同じものが3つあるうえ
各画面の上部を占有していた。ヘッダーは `position: sticky` なので、
スクロールしても残り、どの画面からでも同じ場所で操作できる。
月を持たない画面では何も出さない。
