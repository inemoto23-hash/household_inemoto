...
### 集客ツール（別プロジェクト）に着手する前に確認すること

磨きこみ完了後、SNS への自動投稿ツールを別プロジェクトとして作る予定。
**作り始める前に確認しないと詰むもの**が2つある。

**① X の API は投稿が有料化の方向にある。** 無料枠の月間投稿上限が実装の可否を決めるので、
**ツールの設計より先に現在の枠と価格を確認する。** 上限次第では、
API ではなく手動投稿を支援する形（下書き生成・予約リスト作成）に倒すほうが安上がりになる。
...
**③ 家計簿クラスタは X より Instagram が強い可能性がある。**
手書き家計簿の投稿文化があり、タグの母数が大きい。
X（開発者・節約クラスタ）と Instagram（家計簿クラスタ本体）は**届く層が違う**ので、
どちらに寄せるかを先に決める。
